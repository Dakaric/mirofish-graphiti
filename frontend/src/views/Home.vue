<template>
  <div class="home-container">
    ...
    <div class="main-content">
      ...
      <!-- Unterer Bereich: Zwei-Spalten-Layout -->
      <section class="dashboard-section">
        <!-- Linke Spalte: Status und Schritte -->
        <div class="left-panel">
          ...
          <!-- Vorstellung der Projekt-Simulationsschritte (neuer Bereich) -->
        ...
        <!-- Rechte Spalte: Interaktions-Konsole -->
      ...
      <!-- Datenbank vergangener Projekte -->
      <HistoryDatabase />
    </div>
  </div>
</template>
...
// Computed: Ob das Formular abgeschickt werden kann
const canSubmit = computed(() => {
  return formData.value.simulationRequirement.trim() !== '' && files.value.length > 0
})

// Dateiauswahl auslösen
...
// Simulation starten – sofort weiterleiten, API-Call erfolgt erst auf der Process-Seite
const startSimulation = () => {
  if (!canSubmit.value || loading.value) return
  
  // Daten für späteren Upload zwischenspeichern
  import('../store/pendingUpload.js').then(({ setPendingUpload }) => {
    setPendingUpload(files.value, formData.value.simulationRequirement)
    
    // Sofort zur Process-Seite weiterleiten (spezielle ID kennzeichnet neues Projekt)
    router.push({
      name: 'Process',
...
:root {
  ...
  --border: #E5E5E5;
  /*
    Space Grotesk als primäre Headline-Schrift, JetBrains Mono für Code/Labels.
    Diese Google Fonts müssen in index.html eingebunden sein.
  */
...
.hero-logo {
  max-width: 500px; /* Logo-Größe anpassen */
  width: 100%;
}
...
/* Vorstellung der Projekt-Simulationsschritte */
.steps-container {
  border: 1px solid var(--border);
  padding: 30px;
  position: relative;
...
/* Rechte Interaktions-Konsole */
.right-panel {
  flex: 1.2;
}

.console-box {
  border: 1px solid #CCC; /* Äußere durchgezogene Linie */
  padding: 8px; /* Padding erzeugt einen doppelten Rahmen-Effekt */
}
...
/* Klickbarer Zustand (nicht deaktiviert) */
...
/* Hinweis-Animation: dezenter Rahmen-Puls */
