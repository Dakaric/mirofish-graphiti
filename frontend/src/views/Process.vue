<template>
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <!-- Schrittanzeige in der Mitte -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">Graph-Aufbau</div>
      </div>
    ...
    <div class="main-content">
      <!-- Linke Spalte: Echtzeit-Graph-Anzeige -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            <span class="header-deco">◆</span>
            <span class="header-title">Echtzeit-Knowledge-Graph</span>
          </div>
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? 'Vollbild verlassen' : 'Vollbild'">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph-Visualisierung (zeigt sobald Daten vorhanden) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Hinweis: Aufbau läuft -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              Wird in Echtzeit aktualisiert ...
            </div>

            <!-- Detail-Panel für Knoten/Kanten -->
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">Graph-Daten werden geladen ...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Nach Abschluss startet der Graph-Aufbau automatisch</p>
          </div>

          <!-- Aufbau läuft, aber noch keine Daten -->
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">Daten erscheinen gleich ...</p>
          </div>
      ...
      <!-- Rechte Spalte: Details des Aufbau-Workflows -->
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          <span class="header-icon">▣</span>
          <span class="header-title">Aufbau-Workflow</span>
        </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Nach dem Upload analysiert das LLM die Dokumente und erzeugt automatisch eine Ontologie für die Meinungs-Simulation (Entitäts- und Beziehungstypen).
                </div>
              </div>

              <!-- Fortschritt der Ontologie-Generierung -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">Fortschritt</div>
              ...
              <!-- Bereits generierte Ontologie-Informationen -->
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">Generierte Entitätstypen ({{ projectData.ontology.entity_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Beschreibung</div>
                <div class="detail-content">
                  Auf Basis der generierten Ontologie werden Dokumente in Chunks zerlegt und über die Zep-API als Knowledge-Graph aufgebaut – inkl. Extraktion von Entitäten und Beziehungen.
                </div>
              </div>

              <!-- Warten auf Abschluss der Ontologie -->
              ...
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">Aufbau-Fortschritt</div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  <div class="result-item">
                    ...
                    <span class="result-label">Entitäts-Knoten</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">Beziehungs-Kanten</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">Entitätstypen</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Bereit für den nächsten Schritt</div>
              </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">Projekt-Informationen</span>
          </div>
...
// Aktuelle Projekt-ID (kann sich von 'new' zur realen ID ändern)
...
const ontologyProgress = ref(null) // Fortschritt der Ontologie-Generierung
const currentPhase = ref(-1) // -1: Upload, 0: Ontologie-Generierung, 1: Graph-Aufbau, 2: abgeschlossen
const selectedItem = ref(null) // Aktuell ausgewählter Knoten oder Kante
...
const goToNextStep = () => {
  // TODO: Zum Umgebungsaufbau wechseln
  alert('Umgebungsaufbau noch in Entwicklung ...')
}
...
// Initialisierung – neues oder bestehendes Projekt verarbeiten
const initProject = async () => {
  const paramProjectId = route.params.projectId

  if (paramProjectId === 'new') {
    // Neues Projekt: anstehende Upload-Daten aus dem Store holen
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Keine hochzuladenden Dateien gefunden – bitte zur Startseite zurückkehren und erneut versuchen.'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Phase: Ontologie-Generierung
    ontologyProgress.value = { message: 'Dateien werden hochgeladen und analysiert ...' }
    ...
    // Ontology-Generate-API aufrufen
    const response = await generateOntology(formDataObj)

    if (response.success) {
      // Anstehende Upload-Daten löschen
      clearPendingUpload()
      ...
      // Graph-Aufbau automatisch starten
    ...
    error.value = 'Projekt-Initialisierung fehlgeschlagen: ' + (err.message || 'unbekannter Fehler')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Polling für laufenden Aufbau-Task fortsetzen
      if (response.data.status === 'graph_building' && response.data.graph_build_task_id) {
        currentPhase.value = 1
        startPollingTask(response.data.graph_build_task_id)
      }

      // Bereits abgeschlossenen Graph laden
      if (response.data.status === 'graph_completed' && response.data.graph_id) {
        currentPhase.value = 2
        await loadGraph(response.data.graph_id)
      }
    } else {
      error.value = response.error || 'Projekt konnte nicht geladen werden'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'Projekt konnte nicht geladen werden: ' + (err.message || 'unbekannter Fehler')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Graph-Aufbau-Task gestartet ...'

      // task_id für Polling speichern
      const taskId = response.data.task_id

      // Graph-Daten-Polling starten (unabhängig vom Task-Status)
      startGraphPolling()

      // Task-Status-Polling starten
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'Graph-Aufbau konnte nicht gestartet werden'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Graph-Aufbau konnte nicht gestartet werden: ' + (err.message || 'unbekannter Fehler')
    buildProgress.value = null
  }
}

// Polling-Timer für Graph-Daten
let graphPollTimer = null

// Polling für Graph-Daten starten
const startGraphPolling = () => {
  // Sofort einmal abrufen
  fetchGraphData()

  // Alle 10 Sekunden Graph-Daten neu laden
...
// Polling für Graph-Daten stoppen
...
const fetchGraphData = async () => {
  try {
    // Zuerst Projekt-Info holen, um graph_id zu erhalten
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('Graph-Aufbau abgeschlossen, vollständige Daten werden geladen ...')

        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2

        // Fortschritt auf "abgeschlossen" setzen
        buildProgress.value = {
          progress: 100,
        ...
        if (projectResponse.success) {
          ...
          // Vollständigen Graph laden
          if (projectResponse.data.graph_id) {
            console.log('Vollständiger Graph wird geladen:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph-Aufbau fehlgeschlagen: ' + (task.error || 'unbekannter Fehler')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text('Warten auf Graph-Daten ...')
    return
  }
  
  // Knoten-Map zur Namenssuche aufbauen
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Originaldaten beibehalten
  }))

  // Knoten-ID-Set zum Filtern gültiger Kanten
  ...
  // Kanten zeichnen (inkl. transparenter Klick-Linie)
  ...
  // Sichtbare schmale Linie
  const link = linkGroup.append('line')
    .attr('stroke', '#ccc')
    .attr('stroke-width', 1.5)
    .attr('stroke-opacity', 0.6)
  
  // Transparente breite Linie für Klick-Trefferfläche
  ...
  // Klick auf leere Fläche schließt das Detail-Panel
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // Position aller Kanten aktualisieren (sichtbare und transparente Klick-Linie)
    ...
    // Position der Kantenbeschriftung aktualisieren
    linkLabel
...
// Änderungen der Graph-Daten beobachten
watch(graphData, () => {
  if (graphData.value) {
...
/* Beziehungsanzeige in Kanten-Details */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
