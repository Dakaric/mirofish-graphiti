...
const chatHistoryCache = ref({}) // Cache aller Chatverläufe: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// Aktuellen Chatverlauf in den Cache speichern
...
const selectReportAgentChat = () => {
  // Aktuellen Chatverlauf speichern
  saveChatHistory()
  ...
  // Chatverlauf des Report-Agents wiederherstellen
...
const selectAgent = (agent, idx) => {
  ...
  // Chatverlauf dieses Agents wiederherstellen
...
const renderMarkdown = (content) => {
  ...
  // Listen verarbeiten – Sublisten unterstützen
  ...
  // Ungeordnete Listen umschließen
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Geordnete Listen umschließen
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')
  
  // Whitespace zwischen Listen-Items bereinigen
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Whitespace nach öffnendem Listen-Tag bereinigen
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Whitespace vor schließendem Listen-Tag bereinigen
  ...
  // <p><br> direkt vor Block-Elementen entfernen (entsteht durch überflüssige Leerzeilen)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Aufeinanderfolgende <br>-Tags bereinigen
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // <br> vor öffnendem <p> direkt nach Block-Element bereinigen
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Nummerierung nicht zusammenhängender geordneter Listen reparieren: bei durch Absätze getrennten Einzelitem-<ol>s fortlaufend nummerieren
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Chatverlauf automatisch in den Cache speichern
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Fragender' : 'Du'}: ${msg.content}`)
      .join('\n')
    prompt = `Hier ist unser bisheriger Dialog:\n${historyContext}\n\nMeine neue Frage lautet: ${message}`
  }
  ...
  if (res.success && res.data) {
    // Korrekter Datenpfad: res.data.result.results ist ein Objekt-Dictionary
    // Format: {"twitter_0": {...}, "reddit_0": {...}} oder einplattformig {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Objekt-Dictionary in Array umwandeln, bevorzugt Antworten von Reddit
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Bevorzugt Reddit-Antwort verwenden, ansonsten Twitter
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Objekt-Dictionary in Array-Format umwandeln
...
/* Left Panel - Report Style (identisch mit Step4Report.vue) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Nummerierung geordneter Listen reparieren – CSS-Counter für fortlaufende Nummerierung mehrerer ol */
.message-text {
...
/* Zitat-Style für Chat-/Fragebogenbereich */
