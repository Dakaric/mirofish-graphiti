<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 02: Agent-Personas generieren -->
      ...
      <!-- Step 03: Konfiguration für beide Plattformen generieren -->
      ...
      <!-- Step 04: Initiale Aktivierungs-Orchestrierung -->
      ...
      <!-- Step 05: Vorbereitung abgeschlossen -->
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Konfiguration der Simulationsrunden – nur sichtbar, wenn die Konfiguration fertig und die Rundenzahl berechnet ist -->
...
const props = defineProps({
  simulationId: String,  // Wird vom Parent übergeben
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: Initialisierung, 1: Personas generieren, 2: Konfiguration generieren, 3: abgeschlossen
...
// Log-Deduplikation: speichert die zuletzt geloggten Schlüsselinfos
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Konfiguration der Simulationsrunden
const useCustomRounds = ref(false) // Standardmäßig automatische Rundenzahl
const customMaxRounds = ref(40)   // Empfohlener Standardwert: 40 Runden

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'Agent-Personas generieren' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'Simulationskonfiguration generieren' || newStage === 'generating_config') {
    phase.value = 2
    // In Konfigurations-Phase eingetreten, Polling starten
    if (!configTimer) {
      addLog(t('log.startGeneratingConfig'))
      startConfigPolling()
    }
  } else if (newStage === 'Simulations-Skripte vorbereiten' || newStage === 'copying_scripts') {
    phase.value = 2 // gehört weiterhin zur Konfigurations-Phase
  }
})

// Automatisch generierte Rundenzahl aus der Konfiguration berechnen (kein hartkodierter Default)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Konfiguration noch nicht generiert
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Konfigurationsdaten unvollständig
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Maximalrunden mindestens 40 (Empfehlung), sonst Slider-Range fehlerhaft
...
// Username anhand der agent_id ermitteln
...
// Gesamtzahl der mit allen Personas verknüpften Themen berechnen
...
// Klick auf "Simulation starten" verarbeiten
const handleStartSimulation = () => {
  // Parameter für den Parent aufbauen
  const params = {}

  if (useCustomRounds.value) {
    // Nutzer hat Rundenzahl manuell gesetzt → max_rounds übergeben
    params.maxRounds = customMaxRounds.value
    addLog(t('log.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // Nutzer behält die automatisch generierte Rundenzahl → max_rounds nicht übergeben
...
// Vorbereitung der Simulation automatisch starten
const startPrepareSimulation = async () => {
  ...
  // Schritt 1 als abgeschlossen markieren und Schritt 2 starten
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Erwartete Gesamtzahl der Agents direkt setzen (aus Prepare-Response)
      ...
      startPolling()
      // Profile-Polling in Echtzeit starten
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Stage-Informationen parsen und detaillierte Logs ausgeben
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''
        
        // Detaillierte Fortschritts-Logs ausgeben (Duplikate vermeiden)
        ...
        // Stage aus Nachricht extrahieren
        const match = data.message.match(/\[(\d+)\/(\d+)\]\s*([^:]+)/)
        if (match) {
          currentStage.value = match[3].trim()
        }
        // Nachrichten-Log ausgeben (Duplikate vermeiden)
    ...
    console.warn('Status-Polling fehlgeschlagen:', err)
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Nur aktualisieren, wenn die API einen gültigen Wert liefert, sonst nicht überschreiben
      if (res.data.total_expected) {
        expectedTotal.value = res.data.total_expected
      }
      
      // Entitäts-Typen extrahieren
      const types = new Set()
      ...
      // Logs zum Profile-Generierungsfortschritt nur bei Anzahländerung ausgeben
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        // Wenn alle Profile generiert wurden
    ...
    console.warn('Abruf der Profiles fehlgeschlagen:', err)
  }
}

// Konfigurations-Polling
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Stage-Logs der Konfigurationsgenerierung ausgeben (Duplikate vermeiden)
      ...
      // Wenn die Konfiguration generiert wurde
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog(t('log.configComplete'))

        // Detaillierte Konfigurations-Zusammenfassung anzeigen
        if (data.summary) {
        ...
        // Details der Zeitkonfiguration anzeigen
    ...
    console.warn('Abruf der Config fehlgeschlagen:', err)
  }
}
...
const loadPreparedData = async () => {
  ...
  // Profiles ein letztes Mal abrufen
  await fetchProfilesRealtime()
  addLog(t('log.loadedAgentProfiles', { count: profiles.value.length }))

  // Konfiguration über das Realtime-Endpoint abrufen
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Konfiguration noch nicht generiert → Polling starten
...
onMounted(() => {
  // Vorbereitungs-Prozess automatisch starten
  if (props.simulationId) {
...
/* Style für die Konfiguration der Simulationsrunden */
