          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Fallback: unbekannter Typ oder vorhandener content, der oben nicht behandelt wurde -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Maximale Rundenzahl aus Step 2
  minutesPerRound: {
    type: Number,
    default: 30 // Standardmäßig 30 Minuten pro Runde
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // Alle Aktionen (inkrementell akkumuliert)
const actionIds = ref(new Set()) // ID-Set zur Duplikat-Erkennung
const scrollContainer = ref(null)

// Computed
// Aktionen chronologisch anzeigen (neueste am Ende, also unten)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Aktionsanzahl pro Plattform
...
// Verstrichene Simulationszeit formatieren (aus Runden und Minuten pro Runde berechnen)
...
// Verstrichene Simulationszeit auf Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Verstrichene Simulationszeit auf Reddit
...
// Alle States zurücksetzen (für einen Neustart der Simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Eventuell laufendes Polling stoppen
}
...
const doStartSimulation = async () => {
  ...
  // Erst alle States zurücksetzen, damit die vorherige Simulation keinen Einfluss hat
  resetAllState()
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Dynamische Graph-Updates aktivieren
    }
    ...
    if (res.success && res.data) {
      ...
      startError.value = res.error || 'Start fehlgeschlagen'
...
// Letzte Runde pro Plattform tracken, um Änderungen zu erkennen und zu loggen
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Rundenwechsel pro Plattform separat erkennen und loggen
      ...
      // Prüfen, ob Simulation abgeschlossen ist (über runner_status oder Plattform-Completion)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Zusätzlicher Check: Falls das Backend runner_status noch nicht aktualisiert hat, die Plattform aber Completion meldet,
      // wird über twitter_completed und reddit_completed geprüft.
    ...
    console.warn('Abfrage des Run-Status fehlgeschlagen:', err)
  }
}

// Prüfen, ob alle aktivierten Plattformen abgeschlossen sind
const checkPlatformsCompleted = (data) => {
  // Ohne Plattformdaten: false zurückgeben
  if (!data) return false

  // Completion-Status pro Plattform prüfen
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true

  // Wenn mindestens eine Plattform fertig ist, prüfen, ob alle aktivierten Plattformen fertig sind.
  // Eine Plattform gilt als aktiviert, wenn actions_count > 0, running aktiv ist oder sie completed gemeldet hat.
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted

  // Keine Plattform aktiviert → false
  if (!twitterEnabled && !redditEnabled) return false

  // Prüfen, ob alle aktivierten Plattformen fertig sind
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Vollständige Aktionsliste über all_actions abrufen
      const serverActions = res.data.all_actions || []
      
      // Neue Aktionen inkrementell hinzufügen (mit Deduplikation)
      ...
      // Kein Auto-Scroll, damit der Nutzer die Timeline frei betrachten kann.
      // Neue Aktionen werden unten angehängt.
    }
  } catch (err) {
    console.warn('Abfrage des Detail-Status fehlgeschlagen:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Weiterleitung zur Report-Seite
