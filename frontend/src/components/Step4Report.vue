    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next-Step-Button – nach Abschluss anzeigen -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Inhalt fertig, Sektion möglicherweise noch nicht komplett) -->
                  ...
                  <!-- Section Complete (Sektion vollständig generiert) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Bei finaler Antwort einen Sonder-Hinweis anzeigen -->
...
const toggleRawResult = (timestamp, event) => {
  // Buttonposition relativ zum Viewport speichern
  ...
  // Nach DOM-Update Scrollposition anpassen, damit der Button an derselben Stelle bleibt
...
const toggleSectionCollapse = (idx) => {
  // Nur abgeschlossene Sektionen können eingeklappt werden
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Glühbirne – steht für Insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Globus – steht für Panorama-Suche
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Personen – steht für Dialog
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Blitz – steht für Schnelligkeit
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Diagramm – steht für Statistik
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Datenbank – steht für Entitäten
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Analyse-Frage extrahieren (Backend-LLM liefert deutsche Labels)
    const queryMatch = text.match(/Analysefrage:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Prognoseszenario extrahieren
    const reqMatch = text.match(/Prognoseszenario:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Statistiken extrahieren (Format "- Relevante Prognose-Fakten: X")
    const factMatch = text.match(/Relevante Prognose-Fakten:\s*(\d+)/)
    const entityMatch = text.match(/Beteiligte Entitäten:\s*(\d+)/)
    const relMatch = text.match(/Beziehungsketten:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    // Sub-Queries vollständig extrahieren (keine Begrenzung)
    const subQSection = text.match(/### Analysierte Teilfragen\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }

    // Schlüssel-Fakten vollständig extrahieren (keine Begrenzung)
    const factsSection = text.match(/### \[Kernfakten\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Kern-Entitäten vollständig extrahieren – inkl. Zusammenfassung und Anzahl verwandter Fakten
    const entitySection = text.match(/### \[Kernentitäten\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Entitäts-Blöcke an "- **" splitten
      const entityBlocks = entityText.split(/\n(?=- \*\*)/).filter(b => b.trim().startsWith('- **'))
      result.entities = entityBlocks.map(block => {
        const nameMatch = block.match(/^-\s*\*\*(.+?)\*\*\s*\((.+?)\)/)
        const summaryMatch = block.match(/Zusammenfassung:\s*"?(.+?)"?(?:\n|$)/)
        const relatedMatch = block.match(/Verwandte Fakten:\s*(\d+)/)
        return {
    ...
    // Beziehungsketten vollständig extrahieren (keine Begrenzung)
    const relSection = text.match(/### \[Beziehungsketten\]\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Query extrahieren
    const queryMatch = text.match(/Anfrage:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Statistiken extrahieren
    const nodesMatch = text.match(/Knotenzahl gesamt:\s*(\d+)/)
    const edgesMatch = text.match(/Kantenzahl gesamt:\s*(\d+)/)
    const activeMatch = text.match(/Aktuell gültige Fakten:\s*(\d+)/)
    const histMatch = text.match(/Historische\/abgelaufene Fakten:\s*(\d+)/)
    ...
    // Aktuell gültige Fakten vollständig extrahieren (keine Begrenzung)
    const activeSection = text.match(/### \[Aktuell gültige Fakten\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Nummerierung und Anführungszeichen entfernen
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }

    // Historische bzw. abgelaufene Fakten vollständig extrahieren (keine Begrenzung)
    const histSection = text.match(/### \[Historische\/abgelaufene Fakten\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Beteiligte Entitäten vollständig extrahieren (keine Begrenzung)
    const entitySection = text.match(/### \[Beteiligte Entitäten\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Interview-Thema extrahieren
    const topicMatch = text.match(/\*\*Thema:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()

    // Interview-Teilnehmerzahl extrahieren (z. B. "5 / 9 Simulations-Agents")
    const countMatch = text.match(/\*\*Befragte:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Begründung für die Auswahl der Interview-Teilnehmer extrahieren
    const reasonMatch = text.match(/### Begründung der Auswahl der Befragten\n([\s\S]*?)(?=\n---\n|\n### Interview-Protokoll)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Begründungen pro Person parsen
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1: "Zahl. **Name (index=X)**: Begründung"
        // Beispiel: 1. **Alumnus_345 (index=1)**: Als Wuhan-University-Alumnus ...
        ...
        // Format 2: "- Auswahl Name (index X): Begründung" (deutsches Backend-Token "Auswahl" als Marker)
        // Beispiel: - Auswahl Elternteil_601 (index 0): Als Vertreter der Elterngruppe ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Auswahl\s+([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // Format 3: "- **Name (index X)**: Begründung"
        // Beispiel: - **Elternteil_601 (index 0)**: Als Vertreter der Elterngruppe ...
        if (!headerMatch) {
        ...
        if (name) {
          // Begründung der vorherigen Person speichern
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Neue Person beginnen
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^Nicht ausgewählt|^Zusammenfassend|^Endgültige Auswahl|^Abschließend/i)) {
          // Folgezeile der Begründung (Schluss-/Zusammenfassungs-Absatz ausschließen)
          currentReason.push(line.trim())
        }
      }
      
      // Begründung der letzten Person speichern
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Jeden Interview-Eintrag extrahieren
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Fragen anhand der Nummerierung trennen
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Wenn die erste Frage mit "1." beginnt, gesondert behandeln
      ...
      // Antworten extrahieren – getrennt nach Twitter und Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Kernzitate|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Twitter- und Reddit-Antworten trennen
        const twitterMatch = answerText.match(/\[Antwort Twitter-Plattform\]\n?([\s\S]*?)(?=\[Antwort Reddit-Plattform\]|$)/)
        const redditMatch = answerText.match(/\[Antwort Reddit-Plattform\]\n?([\s\S]*?)$/)
        ...
        // Fallback-Logik für Plattformen (alte Formate mit nur einem Plattform-Marker)
        if (!twitterMatch && redditMatch) {
          // Nur Reddit-Antwort: nur kopieren, wenn es kein Platzhalter ist
          if (interview.redditAnswer && interview.redditAnswer !== '(keine Antwort von dieser Plattform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(keine Antwort von dieser Plattform)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Kein Plattform-Marker (sehr altes Format) – gesamten Text als Antwort übernehmen
          interview.twitterAnswer = answerText
        }
      }
      
      // Schlüsselzitate extrahieren (mehrere Anführungszeichenformate unterstützen)
      const quotesMatch = block.match(/\*\*Kernzitate:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Bevorzugt das Format > "text" matchen
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: matche > "text" oder > \u201Ctext\u201D (typografische Anfuehrungszeichen)
        if (!quoteMatches) {
    ...
    // Interview-Zusammenfassung extrahieren
    const summaryMatch = text.match(/### Interview-Zusammenfassung und Kernaussagen\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Suchanfrage:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Trefferanzahl extrahieren (Format "X relevante Treffer gefunden")
    const countMatch = text.match(/(\d+)\s*relevante Treffer gefunden/)
    if (countMatch) result.count = parseInt(countMatch[1])

    // Verwandte Fakten vollständig extrahieren (keine Begrenzung)
    const factsSection = text.match(/### Relevante Fakten:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }

    // Optional: Kanteninformationen extrahieren (falls vorhanden — Backend liefert aktuell keine separate Kanten-Section, dead-Code-Kandidat)
    const edgesSection = text.match(/### Relevante Kanten:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Optional: Knoteninformationen extrahieren (falls vorhanden — Backend liefert aktuell keine separate Knoten-Section, dead-Code-Kandidat)
    const nodesSection = text.match(/### Relevante Knoten:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Versuche Zeitinformationen zu extrahieren [time - time]
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Pro Frage-Antwort-Paar einen eigenen Plattform-Auswahlstatus halten
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Aktuelle Plattform-Auswahl fuer eine Frage holen
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Plattform-Auswahl fuer eine Frage setzen
    ...
    // Pruefen, ob Text ein Plattform-Platzhalter ist
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(keine Antwort von dieser Plattform)' || t === '[keine Antwort]'
    }

    // Versuchen, die Antwort nach Fragenummern zu splitten
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Unterstuetzt zwei Nummerierungsformate:
      // 1. "Frage X:" oder "Frage X：" (deutsches Backend-Format, siehe zep_tools.py interview prompt)
      // 2. "1. " oder "\n1. " (Zahl + Punkt, Kompatibilitaet zum alten Format)
      let matches = []
      let match

      // Zuerst das "Frage X:" Format versuchen
      const cnPattern = /(?:^|[\r\n]+)Frage\s*(\d+)[：:]\s*/g
      ...
      // Falls kein Match, Fallback auf "Zahl." Format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Wenn keine oder nur eine Nummerierung gefunden, gesamte Antwort zurueckgeben
      if (matches.length <= 1) {
      ...
      // Einzelne Teile nach Nummerierung extrahieren
      const parts = []
    ...
    // Antwort fuer eine bestimmte Frage holen
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split erfolgreich und Index gueltig
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Split fehlgeschlagen: erste Frage erhaelt die komplette Antwort, alle anderen leer
      return qIdx === 0 ? answer : ''
    }
    
    // Pruefen, ob eine Frage Antworten von beiden Plattformen hat (Platzhalter ausfiltern)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Beide Plattformen haben echte Antworten (kein Platzhalter) und unterschiedlichen Inhalt
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Plattform-Umschalter (nur sichtbar bei echten Antworten von beiden Plattformen)
                  ...
                  // Expand/Collapse Button (bei Platzhaltertext nicht anzeigen)
...
// Aktuell aktiver Schritt (fuer Anzeige im oberen Bereich)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Aktuell aktiven Schritt suchen
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Falls kein aktiver Schritt existiert, den letzten abgeschlossenen zurueckgeben
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Andernfalls den ersten Schritt zurueckgeben
  return steps[0] || { noLabel: '--', title: 'Warten auf Start', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Fuehrende H2-Ueberschrift (## xxx) entfernen, da der Section-Titel bereits aussen angezeigt wird
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Codebloecke verarbeiten
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // Inline-Code verarbeiten
  html = html.replace(/`([^`]+)`/g, '<code class="inline-code">$1</code>')
  
  // Ueberschriften verarbeiten
  ...
  // Listen verarbeiten - mit Unterstuetzung fuer verschachtelte Listen
  ...
  // Unordered-List umschliessen
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Ordered-List umschliessen
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Whitespace zwischen Listenelementen entfernen
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Whitespace nach oeffnendem Listen-Tag entfernen
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Whitespace vor schliessendem Listen-Tag entfernen
  ...
  // Trennlinien verarbeiten
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Zeilenumbrueche verarbeiten - Leerzeile wird Absatztrennung, einzelner Umbruch wird <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // In Absatz-Tag umschliessen
  ...
  // <br>-Tags vor und nach Block-Elementen entfernen
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // <p><br> direkt vor Block-Elementen entfernen (entsteht durch ueberzaehlige Leerzeilen)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Aufeinanderfolgende <br>-Tags reduzieren
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // <br> direkt vor Absatzstart nach einem Block-Element entfernen
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Nummerierung nicht zusammenhaengender Ordered-Lists fixen: einzelne <ol>-Bloecke fortlaufend nummerieren
...
const getLogLevelClass = (log) => {
  ...
  // INFO behaelt Standardfarbe und wird nicht als success markiert
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Section-Generierung abgeschlossen
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Frisch generierte Section automatisch ausklappen
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Loading-Status zuverlaessig entfernen
            emit('update-status', 'completed')
            stopPolling()
            // Scroll-Logik wird zentral im nextTick nach dem Loop verarbeitet
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Bei abgeschlossener Aufgabe nach oben scrollen, sonst nach unten dem neuesten Log folgen
            if (isComplete.value) {
...
// Final-Answer-Inhalt extrahieren - Section-Content aus der LLM-Response holen
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Inhalt innerhalb des <final_answer>-Tags extrahieren
  ...
  // Inhalt nach "Final Answer:" suchen (unterstuetzt mehrere Formate)
  // Format 1: Final Answer:\n\nInhalt
  // Format 2: Final Answer: Inhalt
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Beginnt mit ## oder # oder >, dann moeglicherweise direkter Markdown-Inhalt
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Bei laengerem Inhalt mit Markdown-Format den Gedankenprozess entfernen und zurueckgeben
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Mit "Thought:" beginnenden Gedankenprozess entfernen
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dunkelgrau, aendert sich nicht mit dem Status */
  font-weight: 500;
}
...
/* Console Logs - konsistent mit Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
