...
Endpoint 2: Builds a Standalone Graph via the Zep API.
"""
...
class GraphBuilderService:
    """
    Graph builder service.
    Responsible for calling the Zep API to construct knowledge graphs.
    """
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        def safe_attr_name(attr_name: str) -> str:
            """Convert a reserved name into a safe one"""
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # Zep API requires the Field description
    ...
    def add_text_batches(
        ...
        """Add text to the graph in batches; returns the list of all episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch the full graph data (including detailed information).

        Args:
            graph_id: Graph ID

        Returns:
            Dict containing nodes and edges with timing information, attributes, etc.
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Build a node map to look up names
        node_map = {}
        for node in nodes:
