...
Runs the simulation in the background, records each agent action and supports real-time status monitoring.
"""
...
# Tracks whether the cleanup function has been registered
...
class SimulationRunState:
    ...
    # Per-platform round and simulated time (for parallel display of both platforms)
    ...
    # Platform completion (detected via the simulation_end event in actions.jsonl)
    ...
    # Recent actions (used for live frontend display)
    ...
    def add_action(self, action: AgentAction):
        """Append an action to the recent-actions buffer"""
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Detailed view including recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    2. Parse runtime logs and record every agent action
    3. Expose APIs for live status queries
    4. Support pause/stop/resume operations
    """

    # Directory used to persist run state
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # max simulated rounds (optional, for truncating long simulations)
        ...
        Args:
            ...
            max_rounds: Maximum simulated rounds (optional, used to truncate long runs)
            enable_graph_memory_update: Whether to push agent activity into the Zep graph in real time
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation config missing; call /prepare first")
        ...
        # Truncate when a max round count is supplied
        ...
        # Create a graph memory updater if requested
        ...
        # Choose which script to run (located in backend/scripts/)
        if platform == "twitter":
        ...
        try:
            # Build the run command with full paths.
            # New log layout:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # full path to the config file
            ]

            # Forward the max-rounds CLI argument if requested
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])

            # Open the main log file to avoid stdout/stderr pipe buffers stalling the process
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Configure the child process environment for UTF-8 (important on Windows).
            # Fixes third-party libraries (e.g. OASIS) that open files without an explicit encoding.
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+: all open() calls default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # ensure stdout/stderr are UTF-8

            # Set the working directory to the simulation directory (DB files are written there)
            # Use start_new_session=True to spawn a new process group so os.killpg can stop all children.
            process = subprocess.Popen(
                ...
                bufsize=1,
                env=env,  # pass the UTF-8-aware environment
                start_new_session=True,  # new process group so server shutdown kills all children
            )
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # After the process exits, read the logs one more time
            if os.path.exists(twitter_actions_log):
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    logger.info(f"Graph memory updates stopped: simulation_id={simulation_id}")
                except Exception as e:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end and mark the platform as completed
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform was started, only that one matters;
                                    # if both were started, both must finish.
                                ...
                                # Update round info from the round_end event
                                    ...
                                    # Update per-platform round and simulated time
                                    if platform == "twitter":
                                    ...
                                    # Aggregate round number is the max across platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Aggregate simulated time is the max across platforms
                            ...
                            # If graph memory updates are enabled, push the activity to Zep
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Returns:
            True if all enabled platforms have finished.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, state.simulation_id)
        twitter_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        reddit_log = os.path.join(sim_dir, "reddit", "actions.jsonl")

        # Determine which platforms are enabled (via file existence)
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)

        # Return False if any enabled platform is not yet complete
        if twitter_enabled and not state.twitter_completed:
            return False
        if reddit_enabled and not state.reddit_completed:
            return False

        # At least one platform was enabled and finished
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force kill, /T = terminate the tree (including children)
            ...
            try:
                ...
                try:
                    ...
                    # Force kill
                    logger.warning(f"Process not responding, forcing kill: {simulation_id}")
                    subprocess.run(
            ...
            # We used start_new_session=True, so PGID == PID of the leader
            ...
            try:
                ...
                # If it didn't exit in time, send SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action entries (events like simulation_start, round_start, round_end)
                    ...
                    # Determine platform: prefer the record's platform, fall back to default
    ...
    def get_all_actions(
        ...
        """
        Return the full action history across all platforms (no pagination).
        ...
        Returns:
            Full list of actions, sorted by timestamp (newest first)
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []

        # Read Twitter action file (platform inferred from path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (platform inferred from path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files are missing, fall back to the legacy single-file layout
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # legacy file should already carry the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_actions(
        ...
        """
        Return action history (paginated).
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up the simulation's runtime logs (used to force-restart a simulation).
        ...
        Note: does not delete the config file (simulation_config.json) or profile files.
        ...
        # Files to delete (including DB files)
        files_to_delete = [
            "run_state.json",
            "simulation.log",
            "stdout.log",
        ...
        # Directories to scan (contain action logs)
        ...
        logger.info(f"Simulation logs cleaned up: {simulation_id}, deleted: {cleaned_files}")
    ...
    # Flag to guard against double cleanup
    _cleanup_done = False
    ...
    def cleanup_all_simulations(cls):
        ...
        Invoked on server shutdown to ensure all child processes are terminated.
        """
        ...
        # Skip noise if there is nothing to clean up
        ...
        # Copy the dict to avoid mutating during iteration
            ...
            try:
                if process.poll() is None:  # process still running
                    ...
                    try:
                        ...
                        # Process may have already exited — try direct termination
                        try:
                            process.terminate()
                    ...
                    # Also update state.json with status=stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.info(f"state.json updated to status=stopped: {simulation_id}")
                        else:
    ...
    def register_cleanup(cls):
        """
        Register the cleanup callbacks.

        Invoked when the Flask application starts so all simulation processes are torn down on shutdown.
        """
        ...
        # In Flask debug mode only register cleanup in the reloader child (the process that actually runs the app).
        # WERKZEUG_RUN_MAIN=true means we're in the reloader child.
        # Outside debug mode this env var is absent, so we still need to register.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode only register inside the reloader child; outside debug mode always register.
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # mark as registered so the child doesn't retry
            return

        # Preserve the original signal handlers
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP exists only on Unix (macOS/Linux), not on Windows
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler — first clean up simulation processes, then call the original handler"""
            # Only log when there is something to clean up
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}, starting cleanup...")
            cls.cleanup_all_simulations()

            # Invoke the original handler so Flask exits cleanly
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If the original handler is not callable (e.g. SIG_DFL), fall back to default behavior
                raise KeyboardInterrupt

        # Register atexit as a backup
        atexit.register(cls.cleanup_all_simulations)

        # Register signal handlers (only works on the main thread)
        try:
            ...
            # Not on the main thread — fall back to atexit only
            logger.warning("Cannot register signal handlers (not on main thread); using atexit only")
    ...
    def get_running_simulations(cls) -> List[str]:
        """
        Return the list of currently running simulation IDs.
        """
        running = []
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Return detailed status info about the simulation environment.
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specific platform (optional)
                ...
                - None: in dual-platform simulations interview both platforms and return a combined result
        ...
        Raises:
            ValueError: If the simulation is missing or the environment is not running
            TimeoutError: If waiting for the response times out
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation not found: {simulation_id}")
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or already closed; cannot run interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: List of interviews; each item contains {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: Default platform (optional, overridden by each item's platform)
                - "twitter": default to Twitter only
                - "reddit": default to Reddit only
                - None: dual-platform simulations interview every agent on both platforms
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Sends the same prompt to all agents in the simulation.

        Args:
            simulation_id: Simulation ID
            prompt: Interview prompt (all agents receive the same prompt)
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command so the simulation gracefully exits the command-wait mode.
        ...
        try:
            ...
            # A timeout likely means the environment is currently shutting down
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        """
        Read interview history (from the database).

        Args:
            simulation_id: Simulation ID
            platform: Platform (reddit/twitter/None)
                - "reddit": only Reddit history
                - "twitter": only Twitter history
                - None: history from both platforms
            agent_id: Optional agent ID (only return history for that agent)
        ...
        if platform in ("reddit", "twitter"):
            ...
            # No specific platform — query both
