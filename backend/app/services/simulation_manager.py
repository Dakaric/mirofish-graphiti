...
Manages parallel Twitter and Reddit simulations across both platforms.
Uses preset scripts plus LLM-driven config parameter generation.
"""
...
class SimulationState:
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified state dict (used in API responses)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    1. Read entities from a Zep graph and filter them
    ...
    # Directory used to store simulation data
    ...
    def prepare_simulation(
        ...
        2. Generate an OASIS agent profile for each entity (optional LLM enrichment, parallel-capable)
        3. Use an LLM to generate simulation config parameters (time, activity, posting frequency, ...)
        4. Save the config and profile files
        5. Copy preset scripts to the simulation directory

        Args:
            simulation_id: Simulation ID
            simulation_requirement: Simulation requirement (used for LLM config generation)
            document_text: Original document text (provides background for the LLM)
            ...
            parallel_profile_count: Parallelism for profile generation, default 3
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No matching entities found; verify that the graph was built correctly."
                self._save_simulation_state(state)
                return state
            ...
            # Configure the realtime output path (prefer Reddit JSON format)
            ...
            # Reddit was already saved incrementally during generation; we re-save here for completeness
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter must use CSV format — OASIS requires it
                generator.save_profiles(
                    profiles=profiles,
            ...
            # Note: runner scripts stay in backend/scripts/ and are no longer copied per-simulation.
            # When the simulation starts, simulation_runner runs the scripts from that directory.
    ...
    def get_profiles(self, simulation_id: str, platform: str = "reddit") -> List[Dict[str, Any]]:
        """Return the agent profiles for the simulation"""
        state = self._load_simulation_state(simulation_id)
        if not state:
            raise ValueError(f"Simulation not found: {simulation_id}")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                ...
                f"2. Run the simulation (scripts live under {scripts_dir}):\n"
