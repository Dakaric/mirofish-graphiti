"""Zep Graph paginated reader utility.

Zep's node/edge list endpoints use UUID cursor pagination.
This module wraps the auto-pagination logic (with per-page retries) and
transparently returns the full list to the caller.
"""
...
from app.adapters.graphiti_compat import InternalServerError, Zep
...
def fetch_all_nodes(
    ...
    """Paginate graph nodes, returning at most max_items (default 2000). Each page is retried on failure."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate all graph edges and return the full list. Each page is retried on failure."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
