        ...
        try:
            ...
            for interview in interviews:
                ...
                try:
                    ...
                    print(f"  Warning: cannot resolve Agent {agent_id}: {e}")
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Always reads config from the project root .env (highest priority):
        - LLM_API_KEY: API key
        ...
        # Fall back to config when .env does not provide it
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        # Compute activation probability per agent based on its config
        candidates = []
        for cfg in agent_configs:
            ...
            # Skip if outside the agent's active hours
            if current_hour not in active_hours:
                continue

            # Probabilistic activation based on activity level
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(f"  - Total simulated duration: {total_hours} hours")
        ...
        for round_num in range(total_rounds):
            # Compute the current simulated time
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering waiting-for-commands mode - environment stays alive")
...
async def main():
    ...
    # Initialize logging (fixed file names, old logs cleaned up)
...
def setup_signal_handlers():
    """
    Set up signal handlers so that SIGTERM/SIGINT result in a clean exit.
    Gives the program a chance to release resources (close database, environment, etc.).
    """
